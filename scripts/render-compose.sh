#!/usr/bin/env bash
# Liest Host-Port-Konfiguration aus der Umgebung (z. B. Coolify) und schreibt
# docker-compose.yml aus docker-compose.template.yml. So entstehen in der YAML
# nur noch literale Port-Mappings — vermeidet invalid hostPort / abgeschnittene
# Build-Args bei "docker compose build" mit Doppelpunkt-Werten.
set -euo pipefail
...
# Muss zu livekit.yaml rtc port_range (50000–60000, 10001 Ports) passen
...
echo "render-compose: geschrieben $OUT (Host-UDP $LIVEKIT_HOST_UDP_START–$LIVEKIT_HOST_UDP_END → Container 50000–60000/udp)" >&2
